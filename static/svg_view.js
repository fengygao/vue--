...
var svg_editor = (function (sv){
    ...
    var showInfo=function(data){
        //$(".ys-search-result")  //搜索信息

        $("div.ys-content-info").hide().empty();
        ...
        if(data!=null && typeof data != "undefined" ){
            ...
            if(typeof storeId != "undefined" && storeId!=null && storeId!=""){
                if(storeType == "2"){
                    $(".ys-fixed-point-position").show();  //多经
                }else{
                    //加载项目
                    $.ajax({
                        ...
                        success: function(responseText){
                            if (responseText.isRent == "true") {
                                //已出租商铺信息
                                $("div.ys-content-info").empty().append($("#ys-store-info").tmpl(responseText)).fadeIn(300);
                                ...
                                //未出租商铺信息
                                $("div.ys-content-info").empty().append($("#ys-store-info-base").tmpl(responseText)).fadeIn(300);
                            }
                        }
                    });
                }
                ...
                $("div.ys-content-info").append($("#ys-sel-no-store").tmpl()).fadeIn(300); ///未绑定商铺
                $("div.ys-content-info").empty().append($("#ys-store-info").tmpl()).fadeIn(300);
                option.init(['年 13', '14', '15', '16', '17','18'],[80, 90, 120, 140, 165,180],[140, 155, 170, 200, 220,260],[120,130,98,160,110,98],210);
                echarts.init($(".ys-chart").get(0)).setOption(option);
            }
        }else{
            $("div.ys-content-info").append($("#ys-sel-blank").tmpl()).fadeIn(300); //空态
        }
    ...
    var clickSvg=sv.clickSvg=function(evt) {
        ...
        if (targetId== "svgRoot" || targetId=="svgRoot") {
            ...
            //var offsetY = evt.pageY-$(".ys-graphy").offset().top-10-11;

            var offsetX = evt.clientX-$(".ys-graphy").offset().left-12;
            var offsetY = evt.pageY-$(".ys-graphy").offset().top-11-12;
...
svg_editor.reload=function(svg, type){
    ...
    $("#ys-svg").empty().html(svg).prepend("<defs></defs>");
